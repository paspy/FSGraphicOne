...
bool PresentFrame() {
	...
	if ( bitmap && window && windowDC )// bitmap should be allocated at this point
	{
		...
		{
			char buffer[256];
			sprintf_s(buffer, "Raster Surface. FPS: %d", frameCount - framesPast);
...
void ProcessRasterSurface(unsigned int _width, unsigned int _height,
	...
	if ( window ) {
		...
		while ( msg.message != WM_QUIT ) {	// handles ANY message on this thread (not just ones bound to the HWND)
